      ...
      try {
        ...
        if (isDialogLike && scene.characterId) {
          ...
          if (isPremium && process.env.GOOGLE_AI_API_KEY && process.env.FAL_KEY) {
            ...
            // ── STANDARD: Kling Avatar with previous frame context ──
            try {
              send({ progress: "Generating dialog with Kling Avatar..." });
              videoUrl = await klingAvatar(portrait, audioSegment, fullPrompt, "standard");
              console.log(`[Scene Clip] Generated with Kling Avatar v2 Standard`);
            } catch (falErr) {
              console.error(`[Scene Clip] Kling Avatar failed:`, falErr instanceof Error ? falErr.message : falErr);
              ...
              try {
                ...
                throw new Error(`Kling: ${falErr instanceof Error ? falErr.message : "Fehler"}. Hedra: ${hedraErr instanceof Error ? hedraErr.message : "Fehler"}`);
              }
            }
          } else {
